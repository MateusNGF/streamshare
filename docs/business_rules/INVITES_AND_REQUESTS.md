...
- **Origem:** O Organizador convida um e-mail específico pelo painel. Envia um e-mail com um link único.
...
- **Estrutura:** Depende da configuração do Streaming:
  - **Aprovação Automática Ativada:** Cria diretamente o `Participante` e a `Assinatura` na mesma requisição, de forma atômica. Nenhuma solicitação pendente é criada.
  - **Aprovação Manual (Padrão):** Cria um registro na tabela `Convite` com status `solicitado` (não gera `token`).
- **Validade:** A solicitação manual tem uma validade (SLA) de **48 horas**. Se o Organizador não responder dentro deste prazo, ela expira para o usuário e some do painel do Organizador.
- **Ações:** `requestParticipation` (com bypass se `autoAprovarSolicitacoes` for `true`), `approveRequest`, `rejectRequest`.
...
Este mecanismo resolve o problema da *"Black Box"*, onde o participante ficava esperando indefinidamente por uma resposta do Organizador.
...
1. **WhatsApp Mínimo:** Se o usuário **não** possui WhatsApp em seu perfil, o formulário o obriga a preencher *antes* de finalizar o aceite. (Requisito mínimo para a comunicação do Organizador).
