...
export function ParticipantHistoryLine({ data }: ParticipantHistoryLineProps) {
    ...
    const hasData = data && data.some(d => d.valor > 0 || d.status !== 'n/a');
        ...
        <div className="bg-white p-6 md:p-8 rounded-[32px] border border-gray-100 shadow-sm flex flex-col h-[400px]">
            ...
            {!hasData ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-4 animate-in fade-in duration-700">
                    ...
                    <h4 className="font-bold text-gray-900 mb-1">Sem histórico recente</h4>
                    <p className="text-sm text-gray-400 font-medium max-w-[200px]">Ainda não há dados suficientes para traçar o perfil deste participante.</p>
                </div>
            ) : (
                <>
