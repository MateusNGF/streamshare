...
import { confirmarLotePagamento } from "@/actions/cobrancas";
...
export function ModalPagamentoLote({ isOpen, onClose, lote, isAdmin = false }: ModalPagamentoLoteProps) {
    ...
    const steps = status === "pendente"
        ? ["Resumo & PIX", "Comprovante", "Sucesso"]
        : ["Resumo", "Status", "Comprovante"];
    ...
    const renderSummary = () => (
        <StepContainer step={step} className="space-y-4">
            <div className="bg-zinc-50 rounded-[32px] border border-zinc-100 p-6 relative overflow-hidden">
                ...
                <div className="bg-white rounded-2xl border border-zinc-100 divide-y divide-zinc-50 overflow-hidden shadow-sm">
                    <div className="max-h-[160px] overflow-y-auto scrollbar-hide px-4 py-1">
            ...
            {status === "pendente" && (
                <div className="flex gap-4 items-center bg-white p-5 rounded-[32px] border border-zinc-100 shadow-sm ring-1 ring-zinc-50 group">
    ...
    const renderComprovanteStep = () => (
        <StepContainer step={step} className="space-y-6">
            <StepHeader
                ...
                description={status === "pendente" ? "Clique abaixo para selecionar o arquivo" : "Arquivo oficial do lote"}
            />
                ...
                <div className="space-y-4">
                    {lote.comprovanteUrl ? (
                        ...
                        <div className="bg-zinc-50 rounded-[32px] p-16 text-center border border-dashed border-zinc-200">
                            ...
                            <p className="font-black text-zinc-400 text-xs uppercase tracking-widest">Aguardando Envio</p>
                        </div>
                    )}
                </div>
            )}
